...
$sender_filename_prefix = "static_send_node";
...
#hashの初期化
	...
	$AP_ipaddr  =~ s/.50./.30./;    #有線のIPに置換(XXX.XXX.10.XXX)
	#シーケンス番号
	...
	@ap_period  = split(/\./, $filename_field[5],2);
	#$AP_period  = $ap_period[0];
		
        #対応するipaddrの送信ログファイルをオープン
	#$st_file = sprintf("%s%s/static_send_%s_%s_node%s.log",
	$st_file = sprintf("static_send_data_node%s.log",
	...
	foreach $one_line (@st_line) {		
		...
		printf("ip, $AP_ipaddr,sequence,$AP_sequence,delay(raw),$delay,delay(cor),$delay_correct,($ST_time+$offset*($ST_time-$start))\n");
		$amount_delay         += $delay;
