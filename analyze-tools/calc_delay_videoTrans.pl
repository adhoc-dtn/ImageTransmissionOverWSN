...
#送信側ログファイル
$client_logfilename    = "logfile_sendPacket.log";
$server_logfilename    = "logfile_recvPacket.log";
#受信成功率算出までのフレーム転送回数
$numSend     = 5; 
#送信端末数
$numNodes    = 3; 
...
if($exp_type eq "stat") { 	# 固定周期の場合は算出間隔固定値
    $calc_period = $numSend; #受信成功率算出間隔(秒)
    $max_segment = $all_segment;
    $arg_offset  = 1; #受信パケット保存ファイルの引数番号-1(以降のファイルを処理する)

    $client_logdir         = "inHome_log"; #送信側ログ保存ディレクトリ
...
#	printf("now calc ${countNodes} nodes\n");
	...
	if($ip_in_hash == 0) {  
		...
		if($client_dir_name_old eq $client_logdir) { 
		    #ログ保存ディレクトリと一致したら server ipアドレス抽出
		    $DEST_ipaddr  = $client_dir_name;
		    ...
		    #printf("static period ${static_period}, calc period ${timer_calc} sec\n");
		}
	    ...
	    #printf("client addr ${TARGET_ipaddr}\n");
	    $client_file =~ s/${DEST_ipaddr}/${TARGET_ipaddr}/; #送信元IPアドレスに置換
	    #printf("dest ${DEST_ipaddr}\n");
	    #printf("server log ${server_logfilename} , client log ${client_logfilename}\n");
		...
		if( $SERVER_ipaddr eq $TARGET_ipaddr) {
		    ...
		    #printf("sub_client_line(res %d),${sub_client_line[0]}\n",scalar(@sub_client_line));
			...
			#シーケンス番号
			$CLIENT_image_sequence  = $client_field[7];
			$file_counter  += 1;
			#printf("segment,${segment},ip,${SERVER_ipaddr},(serv_seq),(${SERVER_image_sequence}),(client_seq),(${CLIENT_image_sequence}),${file_counter}\n");
			#画像シーケンス番号・パケット番号の検知
			if (${SERVER_image_sequence}  eq  ${CLIENT_image_sequence}  ) {
			    ...
			    #printf("segment,${segment},ip,${SERVER_ipaddr},(seq),(${SERVER_image_sequence}),${SERVER_time},${CLIENT_time},delay,${delay},delaysum,$hash_addr{$SERVER_ipaddr}{'per_packet_delay_sum'},${hash_addr{$SERVER_ipaddr}{'line_number'}}\n");
