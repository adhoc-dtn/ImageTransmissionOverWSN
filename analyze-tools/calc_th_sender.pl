...
foreach $file (@ARGV) {
	...
	#printf("time,${fields[0]},ip,${fields[5]},packetSize,${fields[3]}\n");
...
#ここで算出
	...
	for($segment = 1; $segment <= $max_segment; $segment++) {
	    ...
	    printf("all,%.3lf,kbps,",
		   $th_sum{$segment});
	    $ave_th_all += $th_sum{$segment};
	    $th_sum{$segment}=0;
	    printf("\n");
	}
	$ave_th_all /= $max_segment;
    }
    printf("all_time_average,${ave_th_all}\n");
}
