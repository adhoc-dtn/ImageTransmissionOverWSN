/*Time-stamp: <Wed Feb 12 10:45:26 JST 2014>*/
...
static void UnicastPacket(struct sendingPacketInfo sPInfo, struct next_hop_addr_info *nextHopAddrInfo)  {
    ...
    FILE          *fp_queue;
    char          *logfile_queue = "logfile_enqueue.log";
    u_char         enq_suc = 0;
    struct timeval current_time;
    int            day,usec;
    ...
    if((fp_queue = fopen(logfile_queue, "a")) == NULL) {
        err(EXIT_FAILURE, "fail to open file %s\n", logfile_queue);
    }
    fprintf(fp_queue,
            "%d.%06d,destIP,%s,destPort,%d,enc_suc,%d,q_len,%d\n"
            ,day
            ,usec
            ,inet_ntoa(nextHopAddrInfo->ipDest)
            ,nextHopAddrInfo->destPort
