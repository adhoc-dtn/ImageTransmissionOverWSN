/* Time-stamp: <Mon Jan 06 15:28:39 JST 2014> */


/*
  本サーバプログラムは固定周期を段階変化させ、ログを取るものである
  common.h のSTATIC/ADAPTIVEにかかわらず
  本プログラムは可変周期制御方式は適用されない
...
/* 周期を固定したまま、受信成功率の計算を行う回数 */
#define PERIOD_FIXED_INTERVAL  (5)
/* 実験終了までの固定周期のパターン数 */
#define NUM_PETTERN            (10)
/* 実験終了までのサイクル数 */
#define NUM_CYCLE_FOR_EXIT     (PERIOD_FIXED_INTERVAL*NUM_PETTERN)
...
void CalcDataRecvRate(int signum)
{
    ...
    /* 本関数の呼び出し回数 (==PERIOD_FIXED_INTERVALのとき0に戻る) */
    static u_short num_call = 0;
    ...
    if (amount_of_send > 0) { //この[条件式]=[転送データ量通知メッセージが来ている] とする
        ...
        if ( num_call >= PERIOD_FIXED_INTERVAL) {
            SendingInterval = (1+ALPHA)*SendingInterval;
            SendingPeriodControl (SendingInterval);
            message_interval_makes_long  = 1;
            message_interval_makes_short = 0;
